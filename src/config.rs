 ...
 impl AgentConfig {
     pub fn load() -> Result<Self> {
         ...
         if path.exists() {
            ...
            let value: serde_json::Value = serde_json::from_str(&contents).context("parse config")?;
            if value.get("config_json").is_some() {
                let signed: SignedConfig = serde_json::from_value(value).context("parse signed config")?;
                verify_signature(&signed)?;
                let cfg: AgentConfig = serde_json::from_str(&signed.config_json).context("parse config json")?;
                return Ok(cfg);
            }
